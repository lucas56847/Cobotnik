...
class RoboModel:
    ...
    @update_coords
    def calibrate_servos(self): #assuming that joints are all aligned prior to beginning cal.
        for i in range(1,7):
    ...
    def send_movement_concurrently(self, angleList):
        i = 1
        for angle in angleList: #check all angles valid before doing any moving.
            if self.check_limits(i,angle):
                self.status.set(self.status.get() + "\n" + str("joint: " + str(i))+
                                f" was sent an illegal angle of {angle}!\n")
                return
            else:
                i += 1
    ...
    def check_limits(self, joint, angle):
        ...
        print(
            f"angle: {angle} min ang: {min_angle}\nmax angle: {max_angle}")
